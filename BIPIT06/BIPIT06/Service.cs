...
namespace BIPIT06
{
    ...
    public class Service : IService
    {
        ...
        public void NewRec(int client_id, int service_id, DateTime shipping_date)
        {
            ...
            sqlConnect.Close();
            Console.WriteLine("Количество записей в БД: " + GetData().Length);
        }

        public void GetStatus(string name, string port, string localPath, string uri, string scheme)
        {
            Console.WriteLine("***** Host Info *****");
            Console.WriteLine("Name: {0}, Port: {1}", name, port);
            Console.WriteLine("LocalPath: {0}, Uri: {1}", localPath, uri);
